...
def create_unified_app(
    ...
    Args:
        visualizer_schema: Deprecated. PostgreSQL schema is now provided via query parameter.
    ...
    # Add dynamic visualizer API routes that accept schema as query parameter
    logger.info("Adding dynamic visualizer routes (schema via query parameter)")

    @app.get("/api/customers")
    async def get_visualizer_customers(schema: str):
        """Get all customers for the specified experiment schema.
        
        Args:
            schema: The experiment schema name to query
        """
        try:
            ...
            raise HTTPException(
                status_code=404 if "does not exist" in str(e) else 500,
                detail=f"Unable to load customers: {str(e)}"
            )

    @app.get("/api/businesses")
    async def get_visualizer_businesses(schema: str):
        """Get all businesses for the specified experiment schema.
        ...
        try:
            ...
            raise HTTPException(
                status_code=404 if "does not exist" in str(e) else 500,
                detail=f"Unable to load businesses: {str(e)}"
            )

    @app.get("/api/marketplace-data")
    async def get_visualizer_marketplace_data(schema: str):
        """Get marketplace data (messages, threads, analytics) for the specified experiment schema.
        ...
        try:
            async with connect_to_postgresql_database(
                ...
                analytics_runner = MarketplaceAnalytics(db)
                await analytics_runner.load_data()
                await analytics_runner.analyze_actions()
                analytics_results = analytics_runner.collect_analytics_results()

                # Add utility to threads with payments
                for thread_key in threads_with_payments:
                    ...
                    conversation_utility = analytics_runner.calculate_conversation_utility(
                ...
                for business_summary in analytics_results.business_summaries:
                    ...
                    payments_received = 0
                    for customer_payments in analytics_runner.customer_payments.values():
                        for payment in customer_payments:
                            business_id = analytics_runner._find_business_for_proposal(
            ...
            raise HTTPException(
                status_code=404 if "does not exist" in str(e) else 500,
                detail=f"Unable to load marketplace data: {str(e)}"
            )
